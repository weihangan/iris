...
WEIGHT_FILE = os.path.abspath(os.environ.get("GPT_SOVITS_WEIGHT_FILE", "./weight.json"))
os.makedirs(os.path.dirname(WEIGHT_FILE), exist_ok=True)

if os.path.exists(WEIGHT_FILE):
    pass
else:
    with open(WEIGHT_FILE, 'w', encoding="utf-8") as file:json.dump({'GPT':{},'SoVITS':{}},file)
...
# 设备选择：支持 TTS_DEVICE 环境变量（auto/cpu/cuda）
# auto=自动检测（有CUDA用GPU，无则CPU），cpu=强制CPU，cuda=强制GPU（不可用时回退CPU）
_tts_device_pref = os.environ.get("TTS_DEVICE", "auto").lower()
# ★ CPU 强制 fp16 开关：CHAT5_FORCE_HALF=1 时，CPU 推理也用 fp16 加载模型，
#   大幅降低 TTS python 常驻内存（BERT/ssl/GPT/SoVITS 权重减半）。
#   若合成音色异常可置回 0。GPU 模式不受影响。
_chat5_force_half = os.environ.get("CHAT5_FORCE_HALF", "0") == "1"
if _tts_device_pref == "cpu":
    device = "cpu"
    is_half = True if _chat5_force_half else False
elif _tts_device_pref == "cuda":
    if torch.cuda.is_available():
        device = "cuda"
        is_half = eval(os.environ.get("is_half", "True"))
    else:
        print("[TTS] 警告：指定 CUDA 但 torch.cuda.is_available()=False，回退到 CPU", flush=True)
        device = "cpu"
        is_half = True if _chat5_force_half else False
else:  # auto
    if torch.cuda.is_available():
        ...
        device = "cpu"
        is_half = True if _chat5_force_half else False
print(f"[TTS] 设备: {device}, is_half: {is_half}, 偏好: {_tts_device_pref}", flush=True)
punctuation = set(['!', '?', '…', ',', '.', '-'," "])
import gradio as gr
...
# ★ 直接按目标 dtype 从磁盘加载，避免先加载 fp32 全量再转换的峰值内存
#   （fp16 时 BERT large 621MB → 310MB），降低 TTS python 常驻内存。
...
def change_sovits_weights(sovits_path,prompt_language=None,text_language=None):
    ...
    # A changed SoVITS model changes the meaning of all reference latents.
    # Invalidate only the conditioning cache; text/emotion behavior is untouched.
    if "clear_reference_condition_cache" in globals():
        clear_reference_condition_cache()

    with open(WEIGHT_FILE)as f:
        data=f.read()
        data=json.loads(data)
        data["SoVITS"][version]=sovits_path
    with open(WEIGHT_FILE,"w")as f:f.write(json.dumps(data))
...
def change_gpt_weights(gpt_path):
    ...
    t2s_model.eval()
    # GPT/voice changes must not reuse a condition produced by another voice.
    if "clear_reference_condition_cache" in globals():
        clear_reference_condition_cache()
...
def init_bigvgan():
    ...
    if is_half == True:
        ...
        bigvgan_model = bigvgan_model.float().to(device)
...
# Reference conditioning is speaker identity, not emotion.  Keep it across
# segments of the same reply so every segment uses the same speaker condition
# without repeating librosa/SSL/latent/spectrogram extraction.  The cache is
# deliberately small and invalidated when model weights change below.
_reference_condition_cache = {}
_REFERENCE_CACHE_LIMIT = 4


def _reference_cache_key(ref_wav_path, prompt_text, prompt_language):
    try:
        ...
        stamp = (None, None)
    return (
        os.path.abspath(ref_wav_path or ""), stamp,
        str(prompt_text or ""), str(prompt_language or ""),
        str(model_version), str(version), str(device), bool(is_half),
    )


def _put_reference_condition(key, value):
    _reference_condition_cache[key] = value
    while len(_reference_condition_cache) > _REFERENCE_CACHE_LIMIT:
        _reference_condition_cache.pop(next(iter(_reference_condition_cache)))


def clear_reference_condition_cache():
    _reference_condition_cache.clear()
...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut=i18n("不切"), top_k=20, top_p=0.6, temperature=0.6, ref_free = False,speed=1,if_freeze=False,inp_refs=None,sample_steps=8,if_sr=False,pause_second=0.3):
    ...
    reference_key = _reference_cache_key(ref_wav_path, prompt_text, prompt_language) if not ref_free else None
    cached_reference = _reference_condition_cache.get(reference_key) if reference_key else None
    reference_cache_hit = cached_reference is not None
    if reference_cache_hit:
        print("[TTS] reference_cache_hit=1", flush=True)
    if not ref_free:
        with torch.no_grad():
            if cached_reference is not None:
                prompt = cached_reference["prompt"]
                print("[TTS] reference condition cache hit", flush=True)
            else:
    ...
    for i_text,text in enumerate(texts):
        ...
        if model_version!="v3":
            ...
            if(inp_refs):
                for path in inp_refs:
                    try:
                        ...
                        traceback.print_exc()
            if(len(refers)==0):
                if cached_reference is not None and cached_reference.get("refer") is not None:
                    refers = [cached_reference["refer"]]
                else:
                    refers = [get_spepc(hps, ref_wav_path).to(dtype).to(device)]
                    if reference_key:
                        _put_reference_condition(reference_key, {"prompt": prompt, "refer": refers[0]})
            ...
            if cached_reference is not None and cached_reference.get("v3") is not None:
                v3ref = cached_reference["v3"]
                refer = v3ref["refer"]
                fea_ref = v3ref["fea_ref"]
                ge = v3ref["ge"]
                mel2 = v3ref["mel2"]
                T_min = v3ref["T_min"]
            else:
                ...
                mel2=mel2.to(dtype)
                if reference_key:
                    _put_reference_condition(reference_key, {"prompt": prompt, "v3": {
                        "refer": refer, "fea_ref": fea_ref, "ge": ge,
                        "mel2": mel2, "T_min": T_min,
                    }})
        ...
        t1 = ttime()
    print("%.3f\t%.3f\t%.3f\t%.3f\treference_cache_hit=%s" % (
        t[0], sum(t[1::3]), sum(t[2::3]), sum(t[3::3]), reference_cache_hit
    ))
